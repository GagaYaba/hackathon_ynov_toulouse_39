...
- `medical_security_bias_tests.py` : tests sécurité (refus de posologie/diagnostic dangereux) + biais (paires de prompts genre/origine) pour le modèle médical — à exécuter une fois l'adaptateur Colab téléchargé (aucun GPU local disponible pour l'exécuter ici).
